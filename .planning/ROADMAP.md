...
**Plans:** 14 plans (7 shipped + 7 gap-closure plans for 19 UAT gaps / 23 D-43..D-65 decisions) + 04-15/04-16/04-17 placeholders added on 2026-04-11 pending discuss+plan pass
...
- [ ] 04-15-PLAN.md — [GAP, Wave 6] Notation-layer unification (G-66 bug 1 + architectural cleanup): inflectional rule DSL expects romanized input when romanization is enabled and phonemic input otherwise (never surface IPA); class refs (V/C/F/[class]) stay notation-agnostic; deromanize() becomes the single source of rom→IPA conversion including rewrite-rule application at display/storage boundary; retire romanize() (IPA→rom) — replace its call sites with cached rom or forward-projected values; audit all paradigm / morphology / inspiration / word-generator surfaces to flow rom→IPA, never IPA→rom; migrate existing inflectional rule sources if format changes; needs its own discuss+plan pass (design questions: rom-first vs phonemic-first storage for rule DSL, word generator output format, migration strategy, whether to keep romanize() as a read-through cache or delete it)
- [ ] 04-16-PLAN.md — [GAP, Wave 7] Rules list UX + dimension editor extensions + phoneme validation: (a) when no POS is selected in the Inflections sub-tab / rules list, show ALL inflectional rules (currently shows nothing) — small change in the POS filter in `inflections_page.dart` / `rules_page.dart` with the posScopeFilter introduced in 04-13; (b) per-level rename affordance in `dimension_editor_panel.dart` — extend G-11 dimension rename (04-09) so each DimensionLevel row can be renamed in place; (c) "add new level" affordance for an existing dimension — appends a new DimensionLevel row with next-available id to the dimension's levelsJson; (d) non-existent-phoneme highlighting in the rule editor (G-69) — when a rule's DSL source (condition patterns OR operation arguments like SuffixOp affix, AblautOp from/to, etc.) references an IPA character or digraph that is NOT in the current project's phoneme inventory (not in consonants, not in vowels, not in any named natural class), flag that character visually in the rule editor as a possible error with a tooltip explaining the mismatch. Goal: catch notation-mismatch bugs like G-68 (user wrote `+ci` meaning "rom ci" but the engine reads `c` as a literal IPA char that doesn't exist in the inventory). Should also flag in the rules list display as a subtle warning icon on affected rule rows. Scope note: this is distinct from class refs (V, C, [nasal]) which are ALWAYS valid regardless of inventory — only literal phonemes get validated. Needs its own discuss+plan pass for the visual treatment (underline? red squiggle? warning chip?) and whether to surface at save time as a blocker or just a soft warning.
- [ ] 04-17-PLAN.md — [GAP, Wave 8] Intrinsic dimensions per POS: new concept — a dimension can be marked intrinsic for a specific POS via a new join/flag. An intrinsic dim means a word of that POS IS a fixed level (e.g. masculine OR feminine), not derived into all levels; no inflectional rule fires on that dim; the word's intrinsic level must be set at creation time and is stored on the Lexeme. The paradigm viewer filters to the word's actual intrinsic level (no ghost rows for "the feminine form of this masculine noun"). Other dimensions can be CONDITIONED by intrinsic level (Spanish -ar/-ir/-er verb classes pattern). Intrinsic is per-POS — same dimension can be intrinsic for one POS and non-intrinsic for another (noun gender vs adjective gender). For intrinsic dims, the user can still declare "standard form" patterns (e.g. masculine ends with 'o', feminine ends with 'a') via a lightweight "ends with" / "contains" / "starts with" rule that flags words violating the convention as exceptions in the UI (similar to phonotactic violation highlighting). Later v2 work: noun-adjective agreement propagation. Touches schema (new PosDimensionIntrinsic junction or an intrinsic-for-pos flag on Dimensions, plus a per-lexeme intrinsic-level column), word creation UI (force pick intrinsic levels like POS), lexeme storage, paradigm engine filtering, rule condition DSL extension for intrinsic-level conditions, standard-form pattern matcher + exception highlighting. Needs its own discuss+plan pass before execution.
