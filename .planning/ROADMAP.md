...
- [ ] 04-16-PLAN.md — [GAP, Wave 7] Rules list UX + dimension editor extensions: (a) when no POS is selected in the Inflections sub-tab / rules list, show ALL inflectional rules (currently shows nothing) — small change in the POS filter in `inflections_page.dart` / `rules_page.dart` with the posScopeFilter introduced in 04-13; (b) per-level rename affordance in `dimension_editor_panel.dart` — extend G-11 dimension rename (04-09) so each DimensionLevel row can be renamed in place; (c) "add new level" affordance for an existing dimension — appends a new DimensionLevel row with next-available id to the dimension's levelsJson. Needs its own discuss+plan pass only for the visual placement and whether the paradigm viewer should resurface cells for newly-added levels.
