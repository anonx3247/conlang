...
- [ ] 04-15-PLAN.md — [GAP, Wave 6] Notation-layer unification (D-70..D-77, closes G-66/G-67/G-68): phonemic IPA as single canonical storage for MorphologicalRules.source (D-70/D-71); romanization bijection validator + save-time block + project-open banner (D-72); rule editor rom input → deromanize at save / romanize on load, class-ref invariant doc in morphology_dsl.dart (D-73); v9→v10 Drift migration with round-trip classify rewrite pass + migration_log, 04-17 extends same v10 block (D-74); per-surface render-path audit persisted to 04-15-VERIFICATION.md (D-75); sound rule editor asymmetric labels Pattern (phonemic) / Replacement (phonetic) (D-76); delete static MorphologyPreviewPanel + VerticalDivider from rules_page.dart, keep live preview_panel.dart (D-77). 7 tasks, 1 human-verify checkpoint on migration.
...
- [ ] 04-17-PLAN.md — [GAP, Wave 7] Intrinsic dimensions per POS + standard-form patterns + paradigm viewer stacking (D-82..D-99): schema v10 extension (appended to 04-15's block, no v11 bump) adding Dimensions.intrinsic, Lexemes.intrinsicLevelsJson, StandardFormPatterns table; paradigm engine intrinsic-dim short-circuit + cell enumeration filter; refactored coverage matrix (referenced-dim axes, Cartesian cells); rule editor save-time block on sole-intrinsic-binding rules (D-90 locked error copy); dynamic intrinsic sub-form under POS dropdown in word creation + edit (D-92/D-93 with overlap preservation); stacked intrinsic-slice paradigm viewer with pooled lexeme dropdowns + first-matching-lexeme default (D-94/D-95); new StandardFormMatcher reusing PatternCond class-ref expansion; new StandardFormPatternDao + StandardFormPatternDialog accessed from intrinsic level chips; soft-warning ViolationText wiring across dictionary, word detail, paradigm base form, swadesh, inspiration/thesaurus surfaces; dependency-checked level delete unifying lexemes/patterns/rules. 12 tasks, 2 human-verify checkpoints (schema + stacked viewer).
